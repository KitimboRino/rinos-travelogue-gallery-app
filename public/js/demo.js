...
*/
{
    ...
    calcWinsize();
    // Debounce resize so it doesn't fire hundreds of times during a drag-resize
    let resizeTimer;
    window.addEventListener('resize', () => {
        clearTimeout(resizeTimer);
        resizeTimer = setTimeout(calcWinsize, 100);
    });

    // Track the mouse position — passive:true lets the browser scroll without waiting for this handler
    let mousepos = { x: winsize.width / 2, y: winsize.height / 2 };
    window.addEventListener('mousemove', ev => mousepos = getMousePos(ev), { passive: true });
    ...
    class Cursor {
        ...
        render() {
            ...
            for (const key in this.renderedStyles) {
                const next = MathUtils.lerp(this.renderedStyles[key].previous, this.renderedStyles[key].current, this.renderedStyles[key].amt);
                if (Math.abs(next - this.renderedStyles[key].previous) > 0.05) needsUpdate = true;
                this.renderedStyles[key].previous = next;
            }
    ...
    class ImageGrid {
        ...
        spread(animate = false) {
            return new Promise((resolve, reject) => {
                ...
                const rects = this.DOM.imageWrap.map(item => item.getBoundingClientRect());
                this.DOM.imageWrap.forEach((item, idx) => {
    ...
    class MenuItem {
        constructor(el, imageGrid) {
            ...
            // Need to recalculate size and position on window resize (debounced)
            let menuItemResizeTimer;
            window.addEventListener('resize', () => {
                clearTimeout(menuItemResizeTimer);
                menuItemResizeTimer = setTimeout(() => { this.rect = this.DOM.el.getBoundingClientRect(); }, 100);
            });
        }
        setCurrent() {
    ...
    // Detect WebP support once — use .webp versions if available (generated by npm run optimize-images)
    const supportsWebP = (() => {
    ...
    // Given a data-bg path like ./gallery/img-1.jpg, return the best format available
    const getBestSrc = (src) => {
        if (!supportsWebP || !src) return src;
        return src.replace(/\.(jpg|jpeg|JPG|JPEG|png|PNG)$/, '.webp');
    };

    // Load a single background image, falling back to JPEG if WebP 404s
    const loadBg = (el) => new Promise(resolve => {
        const src = el.dataset.bg;
        if (!src) { resolve(); return; }
        const best = getBestSrc(src);
        const img = new Image();
        img.onload = () => { el.style.backgroundImage = `url(${best})`; resolve(); };
        img.onerror = () => {
            // WebP not found on disk — fall back to original
            const fallback = new Image();
            fallback.onload = fallback.onerror = () => { el.style.backgroundImage = `url(${src})`; resolve(); };
            fallback.src = src;
        };
        img.src = best;
    });

    // Apply background-image from data-bg when the element is near the viewport
    ...
    const preloadFonts = () => {
        return new Promise((resolve) => {
            if (typeof WebFont === 'undefined') { resolve(); return; }
            WebFont.load({
                typekit: { id: 'crf4rue' },
                active: resolve,
                inactive: resolve
